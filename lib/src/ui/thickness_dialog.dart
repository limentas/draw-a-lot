...
Future<double> showThicknessDialog(
    BuildContext context, double x, double y, double currentThickness) async {
      ...
      pageBuilder: (context, animation, secondaryAnimation) {
        return new Align(
            ...
            child: Padding(
                ...
                child: Row(
                    children: listButtonsForDialog(x,
                        MediaQuery.of(context).size.width, currentThickness))));
      });
}

List<Widget> listButtonsForDialog(
    double x, double width, double currentThickness) {
  var res = List<Widget>();
  final minimumButtonWidth = 48.0;

  var buttonsCount =
      min(max(((width - x - 20) / minimumButtonWidth).floor(), 1.0), 12.0);
